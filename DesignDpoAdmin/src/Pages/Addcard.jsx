...
function Addcard() {
  ...
  const fetchCard_byid = async () => {
    let res = await fetch(`http://localhost:5000/getCardById/${id}`);
    res = await res.json();
    setCardData(res.data);
    setimgprev(res.data.cardImage);
  }
  ...
  const formSumbitHandle = async (e) => {
    ...
    if (!id) {
      let response = await fetch('http://localhost:5000/addCard', {
        method: 'POST',
        body: data
      });
      ...
      let response = await fetch(`http://localhost:5000/updateCard/${id}`, {
        method: 'PUT',
        body: data
      });
          ...
          <div className=''>
            <div className='bg-white w-[100%] mb-[50px] p-4 h-full rounded-[20px] '>
              <form action="" onSubmit={formSumbitHandle}>
                Main Heading
                <input type="text" name='mainHeading' value={CardData.mainHeading}
                  onChange={(e) => { setCardData({ ...CardData, mainHeading: e.target.value }) }} className='border border-gray-400 px-4 w-full h-[50px] mb-3 mt-2 ' />
                Sub Heading
                <input type="text" name='subHeading' value={CardData.subHeading} onChange={(e) => { setCardData({ ...CardData, subHeading: e.target.value }) }} className='border border-gray-400 w-full h-[50px] mb-3 mt-2 px-4 ' />
                ...
                <div className='flex items-center mt-5  mb-8 gap-2'>
                  <input type="radio" name='cardStatus' checked={CardData.cardStatus === true || true} value={true} onChange={(e) => { setCardData({ ...CardData, cardStatus: e.target.value }) }} className='mx-2 w-[20px] h-[20px] text-[20px]' /> Active
                  <input type="radio" name='cardStatus' checked={CardData.cardStatus === false} value={false} onChange={(e) => { setCardData({ ...CardData, cardStatus: e.target.value }) }} className='mx-2 w-[20px] h-[20px] text-[20px]' /> Deactive
                </div>
